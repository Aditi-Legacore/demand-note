...
def create_tasks_for_job(job_id: str):
    """
    Create a task for each demand file in the job.
    ...
    with db_connect() as conn, conn.cursor() as cur:
        ...
        for df in demand_files:
            # Previously we inserted DUMMY_TASK_SUMMARY into outputSummary at task creation.
            # Keeping this note for reference; summary is now populated after Groq summarization.
            # task_query = """
            ...
            task_query = """
                INSERT INTO "Task" ("id", "jobId", "demandFileId", "fileName", "filePath", "status", "createdAt", "updatedAt")
                VALUES (gen_random_uuid(), %s, %s, %s, %s, 'pending', NOW(), NOW())
            """
            cur.execute(task_query, (job_id, df['id'], df['fileName'], df['filePath']))
    ...
    if output_summary is not None:
        query = """
            ...
            SET "status" = 'completed',
                "outputFilePath" = %s,
                "outputSummary" = %s,
                "promptVersionId" = COALESCE(%s, "promptVersionId"),
                "endTs" = NOW(),
                "editedSummaryTs" = NOW(),
                "updatedAt" = NOW()
            WHERE "id" = %s
        """
        params = (output_directory, output_summary, prompt_version_id, task_id)
    else:
        query = """
            UPDATE "Task"
            SET "status" = 'completed',
            ...
            WHERE "id" = %s
        """
        params = (output_directory, prompt_version_id, task_id)
...
def get_demand_note_id_for_demand_file(demand_file_id: str) -> Optional[str]:
    """
    Fetch demandNoteId for a given DemandFile ID.
    """
    query = """
        SELECT "demandNoteId"
        FROM "DemandFile"
    ...
    with db_connect() as conn, conn.cursor() as cur:
        ...
        if not row:
            return None
        # row can be dict-like or tuple depending on cursor factory
        return row.get("demandNoteId") if isinstance(row, dict) else row[0]


def get_summarized_demand_files_for_job(job_id: str) -> List[Dict]:
    """
    Get demand files for a job that are already summarized.

    Args:
        job_id: Job ID

    Returns:
        List of summarized demand file dictionaries
    """
    query = """
        SELECT df.*
        FROM "DemandFile" df
        JOIN "Job" j ON j."demandNoteId" = df."demandNoteId"
        WHERE j."id" = %s AND df."summaryStatus" = 'summarized'
        ORDER BY df."createdAt" ASC
    """
